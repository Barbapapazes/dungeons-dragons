...
class ZoneEffect(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        # if self.launch:
        #     self.kill()
        #     self.game.map_img.blit(self.game.)


def collisionZoneEffect(person, game, logs):
    for zone in game.zoneEffect:
        if pg.sprite.collide_rect(person, zone):
            if zone.type == "heal":
                logs.add_log("Une personne gagne " + str(zone.dammage)+" PV de la zone magique")
                person.addHp(zone.dammage)
            else:
                logs.add_log("Une personne subit " + str(zone.dammage)+" dégat de la zone magique")
